...
def geocode_query(query):
    ...
    # store negative result to avoid repeated attempts
    cache[key] = None
    save_cache(cache)
    time.sleep(SLEEP_BETWEEN_LOOKUPS)
    return None
...
def geocode_item(item):
    # build a good query from available fields
    parts = []
    if item.get('title'):
        parts.append(item.get('title'))
    if item.get('postal_code'):
        parts.append(item.get('postal_code'))
    if item.get('city'):
        parts.append(item.get('city'))
    # fallback to raw_text short prefix
    if not parts:
        # use first 150 chars of raw_text
        rt = (item.get('raw_text') or '')[:150]
        if rt:
            parts.append(rt)
